...
async def get_novel_status(novel_id: int):
    ...
    db_data = db_manager.load_workflow_snapshot_and_decision_info(novel_id)

    workflow_status = db_data.get("workflow_status", "unknown") if db_data else "unknown"
    current_step = None # This would ideally come from a dedicated field or parsed from snapshot
    last_history = None # This would ideally come from a dedicated field or parsed from snapshot
    error_msg = None # This would ideally come from a dedicated field

    # More accurate status based on DB fields if available
    if db_data:
        full_state_json = db_data.get("full_workflow_state_json")
        if full_state_json:
            try:
                # Lightly parse state if needed, e.g. for last history entry or current step
                # Avoid full deserialization of complex objects if not necessary for status
                # For example, if history is always at the end of the state dict:
                # state_dict = json.loads(full_state_json)
                # if state_dict.get("history"): last_history = state_dict["history"][-1]
                # current_step might be harder to get without more context or specific field
                pass # Placeholder for more detailed parsing if needed
            except json.JSONDecodeError:
                print(f"Warning: Could not parse full_workflow_state_json for novel {novel_id} in status check.")

        if db_data.get("pending_decision_type"): # If it's pending a decision, that's the current step
    ...
    # Fallback logic if 'workflow_status' isn't detailed enough or specific fields are missing
    if workflow_status in ["unknown", "pending", "processing"] and not db_data.get("pending_decision_type"):
        chapters = db_manager.get_chapters_for_novel(novel_id)
        if chapters:
            workflow_status = "chapters_generated"
            current_step = current_step or f"Chapter {len(chapters)} generated."
        else:
            outline_data = db_manager.get_outline_by_id(novel_record.get("active_outline_id")) if novel_record.get("active_outline_id") else None
            if outline_data: # Changed from outline to outline_data
                workflow_status = "outline_generated"
                current_step = current_step or "Outline generation complete."

    error_msg = db_data.get("error_message") if db_data else None # Assuming error_message field might be added to novels table
...
async def get_next_human_decision(novel_id: int):
    ...
    if not decision_info:
        # This could mean novel not found, or the fields just aren't populated.
        # Check if novel exists first for a clearer error.
        novel_check = db_manager.get_novel_by_id(novel_id)
        if not novel_check:
            raise HTTPException(status_code=404, detail=f"Novel with ID {novel_id} not found.")
        # If novel exists but no decision info, means it's not paused or an issue.
        return DecisionPromptResponse(
            novel_id=novel_id,
            decision_type=None,
            prompt_message="No pending human decision found or novel not in a pausable state.",
            options=[],
            workflow_status=novel_check.get("workflow_status", "unknown")
        )
    ...
    if options_json:
        try:
            ...
            # This is an internal error; the user can't fix it.
            # The workflow is likely in an error state or needs admin intervention.
            raise HTTPException(status_code=500, detail="Error processing decision options for novel.")

    if workflow_status and "paused_for" in workflow_status and pending_decision_type and options_list:
        return DecisionPromptResponse(
            novel_id=novel_id,
        ...
        return DecisionPromptResponse(
            novel_id=novel_id,
            decision_type=None,
            prompt_message="No active human decision pending for this novel.",
            options=[],
            workflow_status=workflow_status
        )
...
def resume_novel_workflow_task(novel_id: int, decision_type: str, decision_payload_dict: dict, db_name_for_task: str):
    ...
    db_manager_task = DatabaseManager(db_name=db_name_for_task) # Create instance inside task

    try:
        manager = WorkflowManager(db_name=db_name_for_task) # mode="human" is implicit now from interaction_mode in state
        ...
        # If the workflow paused again for another decision, the decision node itself should have saved the pause state.
        # If it completed or failed, we update here.
        if not final_status.startswith("paused_for_"):
            final_snapshot_json = json.dumps({
                k: v for k, v in final_state_after_resume.items()
                if isinstance(v, (type(None), str, int, float, bool, list, dict))
            })
            db_manager_task.update_novel_status_after_resume(novel_id, final_status, final_snapshot_json)
            print(f"Novel {novel_id} workflow after resume: Status '{final_status}'. Snapshot updated.")
        else:
            # If it paused again, the pause state (including snapshot) was saved by the decision node.
            # We just log here. The status in DB should reflect the new pause state.
            print(f"Novel {novel_id} workflow paused again after resume for: {final_status}")

    except Exception as e:
        print(f"Critical error in resume_novel_workflow_task for novel_id {novel_id}: {e}")
        import traceback
        traceback.print_exc()
        db_manager_task.update_novel_status_after_resume(novel_id, "system_error_resuming_task") # Update status to reflect task error
    finally:
