...
*   **[x] Orchestration & Workflow Layer (OLC - README 4.2)**
    ...
    *   [p] Integration of human input nodes for user decisions: API-driven human mode for Outline, Worldview, and Conflict Review decisions is functional with backend logic and unit tests in place. An initial cycle of (conceptual) E2E testing and refinement for these API flows has been completed (addressing state serialization, API error handling). Continuous E2E testing and further refinement needed.
...
*   **[p] Context Synthesizer Agent (总结智能体 - README 4.3.8)**
    ...
    *   [p] Retrieves context from LoreKeeperAgent and detailed plot from PlotArchitectAgent; uses detailed character profiles. Brief construction includes hierarchical chapter history. RAG queries are now refined using an LLM to generate focused search terms/questions from the current plot summary. (RAG needs real API key & live tuning. Advanced key event summaries for distant chapters TODO).
...
*   [p] ConflictResolutionAgent (内容冲突解决智能体 - README 4.3.x): Implemented LLM-based logic for `attempt_auto_resolve` (rewrites conflicting excerpts if possible) and `suggest_revisions_for_human_review` (generates textual rewrite suggestions). Integrated with WorkflowManager to support auto-mode resolution and human-mode review (including applying specific suggestions or ignoring conflicts via API). Unit tests updated. (Advanced strategies like iterative revision, precise diff-based text application, or handling multiple suggestions per conflict more interactively are TODO).
...
*   **[p] Phase 3.A: Real-Use Enablement - Live Workflow Hardening & Quality Pass (Revised Focus)**
    ...
    *   [p] (Implicit) Intensive Live End-to-End Workflow Testing: CLI (`main.py`) testing ongoing. API E2E test scripts (`scripts/e2e_tests/`) developed and an initial cycle of (conceptual) execution and backend code refinement has been performed. Continuous execution and further refinement based on real-world/simulated usage needed.
...
*   **[p] Phase 2: Web Interface & Advanced Features (as per original issue doc)**
    *   `[p]` Develop the core features of the Web interface:
        ...
        *   [p] FastAPI backend: Core endpoints for async generation, status, KB viz, and human decisions (Outline, Worldview, Conflict Review with granular actions and re-pausing) are implemented. `WorkflowManager` supports these API-driven flows with DB state persistence. Unit tests and E2E test scripts are in place. Initial cycle of (conceptual) E2E testing and backend refinement completed. Further E2E testing and hardening ongoing.
    ...
    *   `[p]` Establish an intelligent conflict detection mechanism:
        ...
        *   [p] Implementation of conflict auto-resolution (Auto-Mode) or user-choice presentation (Human-Mode) in `WorkflowManager`: Auto-mode now calls functional `ConflictResolutionAgent` for LLM-based excerpt rewrites. Human-mode API flow now prepares conflict data with LLM-generated suggestions from `ConflictResolutionAgent` and supports granular user actions (apply specific suggestion, ignore specific conflict, rewrite all remaining, proceed as is) via API, including re-pausing for multi-step review. Unit tests updated. (Handling of manual text edits via API is TODO).
*   **[p] Phase 3: Optimizations & Advanced Features (as per original issue doc)**
    *   `[p]` Optimize context management and cost control:
        ...
        *   [p] Hierarchical Context Management for `ChapterChroniclerAgent` (Implemented initial version of chapter history hierarchy in `ContextSynthesizerAgent`. RAG query generation now uses LLM for more focused lookups. Unit tests updated. Advanced key event summaries for distant chapters TODO).
...
*   **[p] Testing & Quality Assurance (README 11)**
    ...
    *   [p]` End-to-end tests (CLI `main.py` for basic E2E. Python scripts in `scripts/e2e_tests/` cover API-driven Auto-Mode and Human-Mode scenarios. Initial (conceptual) execution cycle completed, leading to backend refinements for state serialization and API error handling. Continuous E2E testing needed).
